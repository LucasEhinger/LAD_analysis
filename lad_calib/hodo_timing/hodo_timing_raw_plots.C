...
void process_chunk(int i_thread, int start, int end, std::vector<TString> &fileNames,
  ...
  for (int side = 0; side < N_SIDES; ++side) {
    for (int plane = 0; plane < N_PLANES; ++plane) {
      T->SetBranchAddress(
          Form("%c.ladhod.%s.%sTdcTime", spec_prefix, plane_names[plane].c_str(), side_names[side].c_str()),
          &tdc_time[side][plane]);
      T->SetBranchAddress(
...
int hodo_timing_raw_plots() {
  ...
  TString outputFileName = Form("raw_hodo_timing_plots_22572_%c.root", spec_prefix);
