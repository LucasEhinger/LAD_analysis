...
enum { kSIG = 0, kBKD = 1, kN = 2 }; // Signal and background indices
const int nSIG_BKD = 3;              // Number of signal and background types
...
void process_chunk(int thread_id, int start, int end, std::vector<TString> &fileNames,
                   ...
                   map<string, TH1 *[n_dxy_cuts][nPlanes][nSIG_BKD]> &hist_map_dxdy_cuts) {
    ...
    for (int i_plane = 0; i_plane < nPlanes; ++i_plane) {
      ...
      for (int i_hit = 0; i_hit < nData_hodo[i_plane]; ++i_hit) {
        ...
        if (time_avg[i_plane][i_hit] >= time_window_sig.min && time_avg[i_plane][i_hit] < time_window_sig.max) {
          sig_bkd_id = kSIG; // signal
        } else if (time_avg[i_plane][i_hit] >= time_window_bkd.min && time_avg[i_plane][i_hit] < time_window_bkd.max) {
          sig_bkd_id = kBKD; // background
        } else {
          sig_bkd_id = kN; // not in any window
        }

        for (int i_cut = 0; i_cut < n_dxy_cuts; ++i_cut) {
          double max_dx      = dx_cuts[i_cut];
          double max_dy      = dy_cuts[i_cut];
          bool matching_x[2] = {fabs(clust_min_dx[0]) < max_dx, fabs(clust_min_dx[1]) < max_dx};
          bool matching_y[2] = {fabs(clust_min_dy[0]) < max_dy, fabs(clust_min_dy[1]) < max_dy};

          hist_map_dxdy_cuts["h_hodo_time"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
          hist_map_dxdy_cuts["h_hodo_adc_amp"][i_cut][i_plane][sig_bkd_id]->Fill(adc_amp_avg[i_plane][i_hit]);

          if (is_punchthrough) {
            hist_map_dxdy_cuts["h_hodo_time_punchthrough"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
          ...
          if (matching_x[0] && matching_y[0]) {
            hist_map_dxdy_cuts["h_hodo_time_GEM0_xy"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_hodo_time_GEM0_xy_punchthrough"][i_cut][i_plane][kSIG]->Fill(
          ...
          if (matching_x[1] && matching_y[1]) {
            hist_map_dxdy_cuts["h_hodo_time_GEM1_xy"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_hodo_time_GEM1_xy_punchthrough"][i_cut][i_plane][kSIG]->Fill(
                  time_avg[i_plane][i_hit]);
          ...
          if (matching_x[0]) {
            hist_map_dxdy_cuts["h_hodo_time_GEM0_x"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_hodo_time_GEM0_x_punchthrough"][i_cut][i_plane][kSIG]->Fill(
                  time_avg[i_plane][i_hit]);
          ...
          if (matching_y[0]) {
            hist_map_dxdy_cuts["h_hodo_time_GEM0_y"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_hodo_time_GEM0_y_punchthrough"][i_cut][i_plane][kSIG]->Fill(
                  time_avg[i_plane][i_hit]);
          ...
          if (matching_x[1]) {
            hist_map_dxdy_cuts["h_hodo_time_GEM1_x"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_hodo_time_GEM1_x_punchthrough"][i_cut][i_plane][kSIG]->Fill(
                  time_avg[i_plane][i_hit]);
          ...
          if (matching_y[1]) {
            hist_map_dxdy_cuts["h_hodo_time_GEM1_y"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
            if (is_punchthrough) {
          ...
          if (matching_x[0] && matching_x[1]) {
            hist_map_dxdy_cuts["h_hodo_time_GEM_all_x"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_hodo_time_GEM_all_x_punchthrough"][i_cut][i_plane][kSIG]->Fill(
                  time_avg[i_plane][i_hit]);

              hist_map_dxdy_cuts["h_hodo_time_diff_GEM_all_x_punchthrough"][i_cut][i_plane][sig_bkd_id]->Fill(
                  diff_time);
          ...
          if (matching_y[0] && matching_y[1]) {
            hist_map_dxdy_cuts["h_hodo_time_GEM_all_y"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_hodo_time_GEM_all_y_punchthrough"][i_cut][i_plane][kSIG]->Fill(
                  time_avg[i_plane][i_hit]);
              hist_map_dxdy_cuts["h_hodo_time_diff_GEM_all_y_punchthrough"][i_cut][i_plane][sig_bkd_id]->Fill(
                  diff_time);
          ...
          if (matching_x[0] && matching_y[0] && matching_x[1] && matching_y[1]) {
            hist_map_dxdy_cuts["h_hodo_time_GEM_all_xy"][i_cut][i_plane][kSIG]->Fill(time_avg[i_plane][i_hit]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_hodo_time_GEM_all_xy_punchthrough"][i_cut][i_plane][kSIG]->Fill(
                  time_avg[i_plane][i_hit]);
              hist_map_dxdy_cuts["h_hodo_time_diff_GEM_all_xy_punchthrough"][i_cut][i_plane][sig_bkd_id]->Fill(
                  diff_time);
          ...
          // GEM Conditional Efficiency histograms
          if (matching_x[0]) {

            hist_map_dxdy_cuts["h_gem_cond_efficiency_all_GEM1_x"][i_cut][i_plane][sig_bkd_id]->Fill(
                golden_track_dxz[1]);
            if (is_punchthrough) {
          ...
          if (matching_x[1]) {
            hist_map_dxdy_cuts["h_gem_cond_efficiency_all_GEM0_x"][i_cut][i_plane][sig_bkd_id]->Fill(
                golden_track_dxz[0]);
            if (is_punchthrough) {
          ...
          if (matching_x[0] && matching_x[1]) {
            hist_map_dxdy_cuts["h_gem_cond_efficiency_good_hit_GEM0_x"][i_cut][i_plane][sig_bkd_id]->Fill(
                golden_track_dxz[0]);
            hist_map_dxdy_cuts["h_gem_cond_efficiency_good_hit_GEM1_x"][i_cut][i_plane][sig_bkd_id]->Fill(
                golden_track_dxz[1]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_gem_cond_efficiency_good_hit_GEM0_x_punchthrough"][i_cut][i_plane][sig_bkd_id]
          ...
          if (matching_y[0] && matching_y[1]) {
            hist_map_dxdy_cuts["h_gem_cond_efficiency_good_hit_GEM0_y"][i_cut][i_plane][sig_bkd_id]->Fill(
                golden_track_dy[0]);
            hist_map_dxdy_cuts["h_gem_cond_efficiency_good_hit_GEM1_y"][i_cut][i_plane][sig_bkd_id]->Fill(
                golden_track_dy[1]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_gem_cond_efficiency_good_hit_GEM0_y_punchthrough"][i_cut][i_plane][sig_bkd_id]
          ...
          if (matching_x[0] && matching_x[1] && matching_y[0] && matching_y[1]) {
            hist_map_dxdy_cuts["h_gem_cond_efficiency_good_hit_GEM0_2D"][i_cut][i_plane][sig_bkd_id]->Fill(
                golden_track_dxz[0], golden_track_dy[0]);
            hist_map_dxdy_cuts["h_gem_cond_efficiency_good_hit_GEM1_2D"][i_cut][i_plane][sig_bkd_id]->Fill(
                golden_track_dxz[1], golden_track_dy[1]);
            if (is_punchthrough) {
              hist_map_dxdy_cuts["h_gem_cond_efficiency_good_hit_GEM0_2D_punchthrough"][i_cut][i_plane][sig_bkd_id]
          ...
          // GEM0_x
          hist_map_dxdy_cuts["h_gem_efficiency_all_GEM0_x"][i_cut][i_plane][sig_bkd_id]->Fill(golden_track_dxz[0]);
          if (is_punchthrough)
          ...
          // GEM0_y
          hist_map_dxdy_cuts["h_gem_efficiency_all_GEM0_y"][i_cut][i_plane][sig_bkd_id]->Fill(golden_track_dy[0]);
          if (is_punchthrough)
          ...
          // GEM1_x
          hist_map_dxdy_cuts["h_gem_efficiency_all_GEM1_x"][i_cut][i_plane][sig_bkd_id]->Fill(golden_track_dxz[1]);
          if (is_punchthrough)
          ...
          // GEM1_y
          hist_map_dxdy_cuts["h_gem_efficiency_all_GEM1_y"][i_cut][i_plane][sig_bkd_id]->Fill(golden_track_dy[1]);
          if (is_punchthrough)
          ...
          // GEM0 2D
          hist_map_dxdy_cuts["h_gem_efficiency_all_GEM0_2D"][i_cut][i_plane][sig_bkd_id]->Fill(golden_track_dxz[0],
                                                                                               golden_track_dy[0]);
          if (is_punchthrough)
          ...
          // GEM1 2D
          hist_map_dxdy_cuts["h_gem_efficiency_all_GEM1_2D"][i_cut][i_plane][sig_bkd_id]->Fill(golden_track_dxz[1],
                                                                                               golden_track_dy[1]);
          if (is_punchthrough)
...
//   nEntries     = 20000;
  ...
  vector<map<string, TH1 *[n_dxy_cuts][nPlanes][nSIG_BKD]>> hist_map_dxdy_cuts_vec(
    ...
    for (int i_plane = 0; i_plane < nPlanes; ++i_plane) {
      for (int i_gem = 0; i_gem < 2; ++i_gem) {
        ...
        hist_map_vec[thread]["h_gem_clust_dxy_punchthrough"][i_gem][i_plane] =
            new TH2D(Form("h_gem_clust_dxy_punchthrough_%d_%s_thread_%d", i_gem, plane_names[i_plane].c_str(), thread),
        ...
        hist_map_vec[thread]["h_gem_sp_dxy_punchthrough"][i_gem][i_plane] =
            new TH2D(Form("h_gem_sp_dxy_punchthrough_%d_%s_thread_%d", i_gem, plane_names[i_plane].c_str(), thread),
      ...
      for (int i_cut = 0; i_cut < n_dxy_cuts; ++i_cut) {
        ...
        hist_map_dxdy_cuts_vec[thread]["h_hodo_time"][i_cut][i_plane][kSIG] =
            new TH1F(Form("h_hodo_time_cut_dx%d_dy%d_%s_thread_%d", dx_cuts[i_cut], dy_cuts[i_cut],
                     ...
                     Form("h_hodo_time_cut_dx%d_dy%d_%s;Hodoscope time (ns);Counts", dx_cuts[i_cut], dy_cuts[i_cut],
                          plane_names[i_plane].c_str()),
                     lad_time.nbins, lad_time.min, lad_time.max);

        hist_map_dxdy_cuts_vec[thread]["h_hodo_time_punchthrough"][i_cut][i_plane][kSIG] =
            new TH1F(Form("h_hodo_time_punchthrough_cut_dx%d_dy%d_%s_thread_%d", dx_cuts[i_cut], dy_cuts[i_cut],
                          plane_names[i_plane].c_str(), thread),
        ...
        hist_map_dxdy_cuts_vec[thread]["h_hodo_time_GEM_all_y"][i_cut][i_plane][kSIG] =
        ...
        hist_map_dxdy_cuts_vec[thread]["h_hodo_time_GEM_all_xy"][i_cut][i_plane][kSIG] =
            new TH1F(Form("h_hodo_time_GEM_all_xy_cut_dx%d_dy%d_%s_thread_%d", dx_cuts[i_cut], dy_cuts[i_cut],
                          plane_names[i_plane].c_str(), thread),
        ...
        for (int i_sigbkd = 0; i_sigbkd < nSIG_BKD; ++i_sigbkd) {
          ...
          hist_map_dxdy_cuts_vec[thread]["h_hodo_adc_amp_GEM0_x_punchthrough"][i_cut][i_plane][i_sigbkd] =
              new TH1D(Form("h_hodo_adc_amp_GEM0_x_punchthrough_"
          ...
          hist_map_dxdy_cuts_vec[thread]["h_hodo_adc_amp_GEM0_y_punchthrough"][i_cut][i_plane][i_sigbkd] =
              new TH1D(Form("h_hodo_adc_amp_GEM0_y_punchthrough_"
                            "cut_dx%d_dy%d_%s_%s_thread_%d",
                            dx_cuts[i_cut], dy_cuts[i_cut], plane_names[i_plane].c_str(), sigbkd_str, thread),
          ...
          hist_map_dxdy_cuts_vec[thread]["h_hodo_adc_amp_GEM1_x_punchthrough"][i_cut][i_plane][i_sigbkd] =
              new TH1D(Form("h_hodo_adc_amp_GEM1_x_punchthrough_"
                            "cut_dx%d_dy%d_%s_%s_thread_%d",
                            dx_cuts[i_cut], dy_cuts[i_cut], plane_names[i_plane].c_str(), sigbkd_str, thread),
          ...
          hist_map_dxdy_cuts_vec[thread]["h_hodo_adc_amp_GEM1_y_punchthrough"][i_cut][i_plane][i_sigbkd] =
              new TH1D(Form("h_hodo_adc_amp_GEM1_y_punchthrough_"
                            "cut_dx%d_dy%d_%s_%s_thread_%d",
                            dx_cuts[i_cut], dy_cuts[i_cut], plane_names[i_plane].c_str(), sigbkd_str, thread),
          ...
          hist_map_dxdy_cuts_vec[thread]["h_hodo_adc_amp_GEM0_xy_punchthrough"][i_cut][i_plane][i_sigbkd] =
              new TH1D(Form("h_hodo_adc_amp_GEM0_xy_punchthrough_"
                            "cut_dx%d_dy%d_%s_%s_thread_%d",
                            dx_cuts[i_cut], dy_cuts[i_cut], plane_names[i_plane].c_str(), sigbkd_str, thread),
          ...
          hist_map_dxdy_cuts_vec[thread]["h_hodo_adc_amp_GEM1_xy_punchthrough"][i_cut][i_plane][i_sigbkd] =
              new TH1D(Form("h_hodo_adc_amp_GEM1_xy_punchthrough_"
                            "cut_dx%d_dy%d_%s_%s_thread_%d",
                            dx_cuts[i_cut], dy_cuts[i_cut], plane_names[i_plane].c_str(), sigbkd_str, thread),
          ...
          hist_map_dxdy_cuts_vec[thread]["h_hodo_adc_amp_GEM_all_x_punchthrough"][i_cut][i_plane][i_sigbkd] =
              new TH1D(Form("h_hodo_adc_amp_GEM_all_x_"
                            "punchthrough_cut_dx%d_dy%d_%s_%s_thread_%d",
                            dx_cuts[i_cut], dy_cuts[i_cut], plane_names[i_plane].c_str(), sigbkd_str, thread),
                       Form("h_hodo_adc_amp_GEM_all_x_punchthrough_"
                            "cut_dx%d_dy%d_%s_%s;ADC Amplitude [%s];Counts",
                            dx_cuts[i_cut], dy_cuts[i_cut], plane_names[i_plane].c_str(), sigbkd_str, sigbkd_str),
                       LAD_ADC.nbins, LAD_ADC.min, LAD_ADC.max);

          hist_map_dxdy_cuts_vec[thread]["h_hodo_adc_amp_GEM_all_y_punchthrough"][i_cut][i_plane][i_sigbkd] =
              new TH1D(Form("h_hodo_adc_amp_GEM_all_y_"
                            "punchthrough_cut_dx%d_dy%d_%s_%s_thread_%d",
                            dx_cuts[i_cut], dy_cuts[i_cut], plane_names[i_plane].c_str(), sigbkd_str, thread),
                       Form("h_hodo_adc_amp_GEM_all_y_punchthrough_"
                            "cut_dx%d_dy%d_%s_%s;ADC Amplitude [%s];Counts",
                            dx_cuts[i_cut], dy_cuts[i_cut], plane_names[i_plane].c_str(), sigbkd_str, sigbkd_str),
                       LAD_ADC.nbins, LAD_ADC.min, LAD_ADC.max);
        ...
        for (int i_plane = 0; i_plane < nPlanes; ++i_plane) {
          if (hist_map_dxdy_cuts_vec[0].count(histName) && hist_map_dxdy_cuts_vec[0][histName][i_cut][i_plane]) {
            for (int i_sigbkd : {kSIG, kBKD}) {
              if (hist_map_dxdy_cuts_vec[0][histName][i_cut][i_plane][i_sigbkd]) {

                if (i_thread == 0) {
                  // Set the title and name for the histogram in the zero thread
                  // Remove "thread_%d" from the title and name
                  TString title = hist_map_dxdy_cuts_vec[i_thread][histName][i_cut][i_plane][i_sigbkd]->GetTitle();
                  TString name  = hist_map_dxdy_cuts_vec[i_thread][histName][i_cut][i_plane][i_sigbkd]->GetName();
                  title.ReplaceAll(Form("_thread_%d", i_thread), "");
                  name.ReplaceAll(Form("_thread_%d", i_thread), "");
                  hist_map_dxdy_cuts_vec[0][histName][i_cut][i_plane][i_sigbkd]->SetTitle(title);
                  hist_map_dxdy_cuts_vec[0][histName][i_cut][i_plane][i_sigbkd]->SetName(name);
                } else {
                  // Add the histogram to the zero thread
                  hist_map_dxdy_cuts_vec[0][histName][i_cut][i_plane][i_sigbkd]->Add(
                      hist_map_dxdy_cuts_vec[i_thread][histName][i_cut][i_plane][i_sigbkd]);
                }
              }
            }
          }
  ...
  for (int i_plane = 0; i_plane < nPlanes; ++i_plane) {
    ...
    for (int i_cut = 0; i_cut < n_dxy_cuts; ++i_cut) {
      ...
      outputFile->mkdir(Form("time/%s/cut_dx%d_dy%d/Diff_Times_BkdSub", plane_names[i_plane].c_str(), dx_cuts[i_cut],
                             dy_cuts[i_cut]));
      outputFile->mkdir(
  ...
  for (int i_plane = 0; i_plane < nPlanes; ++i_plane) {
    ...
    for (int i_cut = 0; i_cut < n_dxy_cuts; ++i_cut) {
      outputFile->cd(Form("time/%s/cut_dx%d_dy%d/Times", plane_names[i_plane].c_str(), dx_cuts[i_cut], dy_cuts[i_cut]));
      hist_map_dxdy_cuts_vec[0]["h_hodo_time"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_punchthrough"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM0_x"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM0_y"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM1_x"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM1_y"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM0_x_punchthrough"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM0_y_punchthrough"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM1_x_punchthrough"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM1_y_punchthrough"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM0_xy"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM1_xy"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM_all_x"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM_all_y"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM_all_xy"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM0_xy_punchthrough"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM1_xy_punchthrough"][i_cut][i_plane][kSIG]->Write();
      hist_map_dxdy_cuts_vec[0]["h_hodo_time_GEM_all_x_punchthrough"][i_cut][i_plane][kSIG]->Write();
      ...
      bkdSubDir = Form("GEM_cond_Efficiency_BkdSub/%s/cut_dx%d_dy%d", plane_names[i_plane].c_str(), dx_cuts[i_cut],
                       dy_cuts[i_cut]);
      noSubDir =
          Form("GEM_cond_Efficiency/%s/cut_dx%d_dy%d", plane_names[i_plane].c_str(), dx_cuts[i_cut], dy_cuts[i_cut]);
      ...
      bkdSubDir =
          Form("GEM_Efficiency_BkdSub/%s/cut_dx%d_dy%d", plane_names[i_plane].c_str(), dx_cuts[i_cut], dy_cuts[i_cut]);
      noSubDir = Form("GEM_Efficiency/%s/cut_dx%d_dy%d", plane_names[i_plane].c_str(), dx_cuts[i_cut], dy_cuts[i_cut]);
      // Create directories for bkdSubDir and noSubDir if they do not exist
      if (!outputFile->GetDirectory(noSubDir.c_str())) {
        outputFile->mkdir(noSubDir.c_str());
      }
