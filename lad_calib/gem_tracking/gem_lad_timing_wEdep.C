...
const int n_edep_cuts               = 5;
const double edep_cuts[n_edep_cuts] = {0.0, 0.5, 1.0, 2.0, 3.0}; // Energy deposition cuts
...
void gem_lad_timing_full(int run_num) {
  ...
  Double_t adc_time_btm[nPlanes][MAX_DATA], adc_time_top[nPlanes][MAX_DATA];
  Double_t adc_counter_btm[nPlanes][MAX_DATA], adc_counter_top[nPlanes][MAX_DATA];
  Double_t adc_amp_btm[nPlanes][MAX_DATA], adc_amp_top[nPlanes][MAX_DATA];
  Double_t adc_integral_btm[nPlanes][MAX_DATA], adc_integral_top[nPlanes][MAX_DATA];
  Int_t nTracks, nTdcTopHits[nPlanes], nTdcBtmHits[nPlanes], nAdcTopHits[nPlanes], nAdcBtmHits[nPlanes];
  ...
  for (int i = 0; i < nPlanes; ++i) {
    ...
    T->SetBranchAddress(Form("Ndata.%c.ladhod.%s.BtmAdcPulseTime", spect_prefix, plane_names[i].c_str()),
                        &nAdcBtmHits[i]);
    T->SetBranchAddress(Form("Ndata.%c.ladhod.%s.TopAdcPulseTime", spect_prefix, plane_names[i].c_str()),
                        &nAdcTopHits[i]);
    T->SetBranchAddress(Form("%c.ladhod.%s.BtmAdcPulseTisme", spect_prefix, plane_names[i].c_str()), &adc_time_btm[i]);
    T->SetBranchAddress(Form("%c.ladhod.%s.BtmAdcCounter", spect_prefix, plane_names[i].c_str()), &adc_counter_btm[i]);
    T->SetBranchAddress(Form("%c.ladhod.%s.TopAdcPulseTimeRaw", spect_prefix, plane_names[i].c_str()),
                        &adc_time_top[i]);
    T->SetBranchAddress(Form("%c.ladhod.%s.TopAdcCounter", spect_prefix, plane_names[i].c_str()), &adc_counter_top[i]);
    T->SetBranchAddress(Form("%c.ladhod.%s.BtmAdcPulseAmp", spect_prefix, plane_names[i].c_str()), &adc_amp_btm[i]);
    T->SetBranchAddress(Form("%c.ladhod.%s.BtmAdcPulseInt", spect_prefix, plane_names[i].c_str()),
                        &adc_integral_btm[i]);
    T->SetBranchAddress(Form("%c.ladhod.%s.TopAdcPulseAmp", spect_prefix, plane_names[i].c_str()), &adc_amp_top[i]);
    T->SetBranchAddress(Form("%c.ladhod.%s.TopAdcPulseInt", spect_prefix, plane_names[i].c_str()),
                        &adc_integral_top[i]);
  }
  ...
  TH1D *h_adc_time_wTrack_top[nPlanes][n_edep_cuts];
  TH1D *h_adc_time_wTrack_btm[nPlanes][n_edep_cuts];
  TH1D *h_adc_amp_wTrack_top[nPlanes][n_edep_cuts];
  TH1D *h_adc_amp_wTrack_btm[nPlanes][n_edep_cuts];
  TH1D *h_adc_int_wTrack_top[nPlanes][n_edep_cuts];
  TH1D *h_adc_int_wTrack_btm[nPlanes][n_edep_cuts];
  TH1D *h_adc_time_all_times_top[nPlanes][n_edep_cuts];
  TH1D *h_adc_time_all_times_btm[nPlanes][n_edep_cuts];
  TH1D *h_adc_amp_all_times_top[nPlanes][n_edep_cuts];
  TH1D *h_adc_amp_all_times_btm[nPlanes][n_edep_cuts];
  TH1D *h_adc_int_all_times_top[nPlanes][n_edep_cuts];
  TH1D *h_adc_int_all_times_btm[nPlanes][n_edep_cuts];
  ...
  for (int i_plane = 0; i_plane < nPlanes; ++i_plane) {
    ...
    h_projy_btm[i_plane]     = new TH1D(Form("h_projy_btm_%d", i_plane), Form("Bottom ProjY Plane %d", i_plane),
                                        TARGED_POS.nbins, TARGED_POS.min, TARGED_POS.max);
    for (int i_edep = 0; i_edep < n_edep_cuts; ++i_edep) {
      h_adc_time_wTrack_top[i_plane][i_edep] =
          new TH1D(Form("h_adc_time_top_%d_%d", i_plane, i_edep),
                   Form("Top ADC Time Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_time_wTrack_btm[i_plane][i_edep] =
          new TH1D(Form("h_adc_time_btm_%d_%d", i_plane, i_edep),
                   Form("Bottom ADC Time Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_amp_wTrack_top[i_plane][i_edep] =
          new TH1D(Form("h_adc_amp_top_%d_%d", i_plane, i_edep),
                   Form("Top ADC Amp Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_amp_wTrack_btm[i_plane][i_edep] =
          new TH1D(Form("h_adc_amp_btm_%d_%d", i_plane, i_edep),
                   Form("Bottom ADC Amp Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_int_wTrack_top[i_plane][i_edep] =
          new TH1D(Form("h_adc_int_top_%d_%d", i_plane, i_edep),
                   Form("Top ADC Int Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_int_wTrack_btm[i_plane][i_edep] =
          new TH1D(Form("h_adc_int_btm_%d_%d", i_plane, i_edep),
                   Form("Bottom ADC Int Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_time_all_times_top[i_plane][i_edep] =
          new TH1D(Form("h_adc_time_all_times_top_%d_%d", i_plane, i_edep),
                   Form("Top ADC All TDC Time Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_time_all_times_btm[i_plane][i_edep] =
          new TH1D(Form("h_adc_time_all_times_btm_%d_%d", i_plane, i_edep),
                   Form("Bottom ADC All TDC Time Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_amp_all_times_top[i_plane][i_edep] =
          new TH1D(Form("h_adc_amp_all_times_top_%d_%d", i_plane, i_edep),
                   Form("Top ADC All Amp Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_amp_all_times_btm[i_plane][i_edep] =
          new TH1D(Form("h_adc_amp_all_times_btm_%d_%d", i_plane, i_edep),
                   Form("Bottom ADC All Amp Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_int_all_times_top[i_plane][i_edep] =
          new TH1D(Form("h_adc_int_all_times_top_%d_%d", i_plane, i_edep),
                   Form("Top ADC All Int Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
      h_adc_int_all_times_btm[i_plane][i_edep] =
          new TH1D(Form("h_adc_int_all_times_btm_%d_%d", i_plane, i_edep),
                   Form("Bottom ADC All Int Plane %d Edep > %d", i_plane, edep_cuts[i_edep]), TDC_TIME_RAW.nbins,
                   TDC_TIME_RAW.min, TDC_TIME_RAW.max);
    }
  }
  ///////////////////////////////////////////////////////////////////
  ...
  for (Long64_t i = 0; i < nEntries; ++i) {
    ...
    for (int i_trk = 0; i_trk < nTracks; ++i_trk) {
      ...
      // Loop through all ADC hits
      for (int i_plane = 0; i_plane < nPlanes; ++i_plane) {
        for (int i_edep = 0; i_edep < n_edep_cuts; ++i_edep) {
          for (int i_top = 0; i_top < nAdcTopHits[i_plane]; ++i_top) {

            if (adc_integral_top[i_plane][i_top] > edep_cuts[i_edep]) {
              TVector3 p_gemhit_0(trk_x[0][i_trk], trk_y[0][i_trk], trk_z[0][i_trk]);
              TVector3 p_gemhit_1(trk_x[1][i_trk], trk_y[1][i_trk], trk_z[1][i_trk]);
              int paddle        = adc_counter_top[i_plane][i_top]-1;
              double dca_xz_top = get_dca_xz_hodo(i_plane, paddle, p_gemhit_0, p_gemhit_1);
              double dca_y_top  = get_dca_y_hodo(i_plane, paddle, p_gemhit_0, p_gemhit_1);
              h_adc_time_all_times_top[i_plane][i_edep]->Fill(adc_time_top[i_plane][i_top]);
              h_adc_amp_all_times_top[i_plane][i_edep]->Fill(adc_amp_top[i_plane][i_top]);
              h_adc_int_all_times_top[i_plane][i_edep]->Fill(adc_integral_top[i_plane][i_top]);
              if (dca_xz_top < DCA_XZ_MAX) {
                h_adc_time_wTrack_top[i_plane][i_edep]->Fill(adc_time_top[i_plane][i_top]);
                h_adc_amp_wTrack_top[i_plane][i_edep]->Fill(adc_amp_top[i_plane][i_top]);
                h_adc_int_wTrack_top[i_plane][i_edep]->Fill(adc_integral_top[i_plane][i_top]);
              }
            }
          }
          for (int i_btm = 0; i_btm < nAdcBtmHits[i_plane]; ++i_btm) {
            if (adc_integral_btm[i_plane][i_btm] > edep_cuts[i_edep]) {
              TVector3 p_gemhit_0(trk_x[0][i_trk], trk_y[0][i_trk], trk_z[0][i_trk]);
              TVector3 p_gemhit_1(trk_x[1][i_trk], trk_y[1][i_trk], trk_z[1][i_trk]);
              int paddle        = adc_counter_btm[i_plane][i_btm]-1;
              double dca_xz_btm = get_dca_xz_hodo(i_plane, paddle, p_gemhit_0, p_gemhit_1);
              double dca_y_btm  = get_dca_y_hodo(i_plane, paddle, p_gemhit_0, p_gemhit_1);
              h_adc_time_all_times_btm[i_plane][i_edep]->Fill(adc_time_btm[i_plane][i_btm]);
              h_adc_amp_all_times_btm[i_plane][i_edep]->Fill(adc_amp_btm[i_plane][i_btm]);
              h_adc_int_all_times_btm[i_plane][i_edep]->Fill(adc_integral_btm[i_plane][i_btm]);
              if (dca_xz_btm < DCA_XZ_MAX) {
                h_adc_time_wTrack_btm[i_plane][i_edep]->Fill(adc_time_btm[i_plane][i_btm]);
                h_adc_amp_wTrack_btm[i_plane][i_edep]->Fill(adc_amp_btm[i_plane][i_btm]);
                h_adc_int_wTrack_btm[i_plane][i_edep]->Fill(adc_integral_btm[i_plane][i_btm]);
              }
            }
          }
        }
  ...
  TDirectory *dir_adc_time    = outputFile->mkdir("adc_time");
  TDirectory *dir_adc_amp     = outputFile->mkdir("adc_amp");
  TDirectory *dir_adc_int     = outputFile->mkdir("adc_int");
  ...
  for (int i_plane = 0; i_plane < nPlanes; ++i_plane) {
    for (int i_edep = 0; i_edep < n_edep_cuts; ++i_edep) {
      h_adc_time_wTrack_top[i_plane][i_edep]->Write();
      h_adc_time_wTrack_btm[i_plane][i_edep]->Write();
      h_adc_time_all_times_top[i_plane][i_edep]->Write();
      h_adc_time_all_times_btm[i_plane][i_edep]->Write();
    }
  }
