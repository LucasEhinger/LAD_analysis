...
const double d0_cut_sig        = 30.0; // Not currently using d0 cut
const double delta_pos_trans_bkd = 400.0;
const double delta_pos_long_bkd  = 400.0;
const double d0_cut_bkd        = 100.0; // Not currently using d0 cut

const int maxTracks = 30;

void gem_sig_bkd(int run_number, int num_replayed) {
  // Set batch mode to suppress graphical output
  gROOT->SetBatch(kTRUE);
  // Open the ROOT file
  TString fileName = Form("/volatile/hallc/c-lad/ehingerl/lad_replay/ROOTfiles/LAD_COIN/PRODUCTION/LAD_COIN_%d_%d.root", run_number, num_replayed);
  ...
  T->SetBranchAddress((branchPrefix + ".goodhit_deltapostrans_0").c_str(), &kin_deltapostrans_0);
  T->SetBranchAddress((branchPrefix + ".goodhit_deltapostrans_1").c_str(), &kin_deltapostrans_1);
  T->SetBranchAddress((branchPrefix + ".goodhit_deltaposlong_0").c_str(), &kin_deltaposlong_0);
  T->SetBranchAddress((branchPrefix + ".goodhit_deltaposlong_1").c_str(), &kin_deltaposlong_1);

  // Create TH2D histograms for delta_pos_long vs delta_pos_trans
  TH2D *h_deltaposlong_vs_deltapostrans_0 =
      new TH2D("h_deltaposlong_vs_deltapostrans_0",
               "Delta Pos Long vs Delta Pos Trans (0);Delta Pos Trans (cm);Delta Pos Long (cm)", deltapos_NBINS,
               deltapos_MIN, deltapos_MAX, deltapos_NBINS, deltapos_MIN, deltapos_MAX);

  TH2D *h_deltaposlong_vs_deltapostrans_1 =
      new TH2D("h_deltaposlong_vs_deltapostrans_1",
               "Delta Pos Long vs Delta Pos Trans (1);Delta Pos Trans (cm);Delta Pos Long (cm)", deltapos_NBINS,
               deltapos_MIN, deltapos_MAX, deltapos_NBINS, deltapos_MIN, deltapos_MAX);

  // Create TH2D histograms for delta_pos_long vs delta_pos_trans with track cut
  TH2D *h_deltaposlong_vs_deltapostrans_trackcut_0 =
      new TH2D("h_deltaposlong_vs_deltapostrans_trackcut_0",
               "Delta Pos Long vs Delta Pos Trans (0) with Track Cut;Delta Pos Trans (cm);Delta Pos Long (cm)",
               deltapos_NBINS, deltapos_MIN, deltapos_MAX, deltapos_NBINS, deltapos_MIN, deltapos_MAX);

  TH2D *h_deltaposlong_vs_deltapostrans_trackcut_1 =
      new TH2D("h_deltaposlong_vs_deltapostrans_trackcut_1",
               "Delta Pos Long vs Delta Pos Trans (1) with Track Cut;Delta Pos Trans (cm);Delta Pos Long (cm)",
               deltapos_NBINS, deltapos_MIN, deltapos_MAX, deltapos_NBINS, deltapos_MIN, deltapos_MAX);
  ...
  for (Long64_t i = 0; i < nEntries; ++i) {
    ...
    for (int k = 0; k < nGoodHits; ++k) {
      h_deltaposlong_vs_deltapostrans_0->Fill(kin_deltapostrans_0[k], kin_deltaposlong_0[k]);
      h_deltaposlong_vs_deltapostrans_1->Fill(kin_deltapostrans_1[k], kin_deltaposlong_1[k]);
      if (nTracks < maxTracks) {
        h_deltaposlong_vs_deltapostrans_trackcut_0->Fill(kin_deltapostrans_0[k], kin_deltaposlong_0[k]);
        h_deltaposlong_vs_deltapostrans_trackcut_1->Fill(kin_deltapostrans_1[k], kin_deltaposlong_1[k]);
      }
      ...
      if (abs(kin_deltaposlong_0[k]) < delta_pos_long_bkd && abs(kin_deltapostrans_0[k]) < delta_pos_trans_bkd) {
        n_background++;
        if (nTracks < maxTracks) {
          n_background_trk_cut++;
        }
      }
      // if (abs(kin_deltaposlong_1[k]) < delta_pos_long_sig && abs(kin_deltapostrans_1[k]) < delta_pos_trans_sig) {
      //   n_signal++;
      //   if (nTracks < maxTracks) {
      //     n_signal_trk_cut++;
      //   }
      // }
      // if (abs(kin_deltaposlong_1[k]) < delta_pos_long_bkd && abs(kin_deltapostrans_1[k]) < delta_pos_trans_bkd) {
      //   n_background++;
      //   if (nTracks < maxTracks) {
      //     n_background_trk_cut++;
      //   }
  ...
  outputFile->cd();
  h_deltaposlong_vs_deltapostrans_0->Write();
  h_deltaposlong_vs_deltapostrans_1->Write();
  h_deltaposlong_vs_deltapostrans_trackcut_0->Write();
  h_deltaposlong_vs_deltapostrans_trackcut_1->Write();
  ...
  c_boxes->cd(1);
  h_deltaposlong_vs_deltapostrans_0->Draw("COLZ");
  ...
  c_boxes->cd(3);
  h_deltaposlong_vs_deltapostrans_trackcut_0->Draw("COLZ");
  redBox_0->Draw("same");
  greenBox_0->Draw("same");
  ...
  latex.DrawLatex(0.1, 0.8, Form("Signal Count (No Track Cut): %d", n_signal));
  latex.DrawLatex(0.1, 0.75, Form("Background Count (No Track Cut): %d", n_background));
  if (n_background > 0) {
    latex.DrawLatex(0.1, 0.7, Form("Signal to Background Ratio (No Track Cut): %.2f", static_cast<double>(n_signal) / n_background));
  } else {
    latex.DrawLatex(0.1, 0.7, "Signal to Background Ratio (No Track Cut): Undefined (Background = 0)");
  }

  latex.DrawLatex(0.1, 0.6, Form("Signal Count (With Track Cut): %d", n_signal_trk_cut));
  latex.DrawLatex(0.1, 0.55, Form("Background Count (With Track Cut): %d", n_background_trk_cut));
  if (n_background_trk_cut > 0) {
    latex.DrawLatex(0.1, 0.5, Form("Signal to Background Ratio (With Track Cut): %.2f", static_cast<double>(n_signal_trk_cut) / n_background_trk_cut));
  } else {
