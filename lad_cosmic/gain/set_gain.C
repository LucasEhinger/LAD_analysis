...
const static string peak_max_type = "landau";      // "landau", mode, or median
const static string fit_file_name = "fit_params.txt";
const static int AMP_MAX_LOWER    = 5;
const static int AMP_MAX_UPPER    = 900;

const static double TARGET_ADC            = 50; // was 100 for cosmic
...
void getPeakPlane(TFile *file, int plane_idx, int run_idx, vector<vector<double>> &amplitudes_btm,
  ...
  for (int i = 0; i < N_BARS; i++) {
    ...
    if (peak_max_type == "median") {
      ...
      TF1 *landau_fit = new TF1("landau_fit", "landau", 0, 1000);
      h1_amp_top->Fit(landau_fit, "Q");
  ...
  void set_gain() {
    ...
    while (getline(fit_params_file, fit_line)) {
      ...
      string plane_id =
          (bar_id.substr(0, 6) == "REFBAR" ? "999" : bar_id.substr(0, 3)); // Handle REFBAR as plane ID 999
    ...
    for (int plane_idx = 0; plane_idx < N_PLANES; plane_idx++) {
      ...
      c1->SaveAs(Form("files/%s/laser_fitted/peak_amp_top_%s_plane_%s.pdf", sub_dir.c_str(),
              peak_max_type.c_str(), plane_names[plane_idx].c_str()));
      ...
      c2->SaveAs(Form("files/%s/laser_fitted/peak_amp_btm_%s_plane_%s.pdf", sub_dir.c_str(),
              peak_max_type.c_str(), plane_names[plane_idx].c_str()));
