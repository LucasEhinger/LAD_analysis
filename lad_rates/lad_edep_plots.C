...
// Not great when it comes to memory management. I wasn't smart enough to multithread with ROOT elegantly, but this
// works.
#include <TCanvas.h>
#include <TChain.h>
#include <TFile.h>
  ...
  for (int i = start; i < end; ++i) {
    ...
    for (int plane = 0; plane < N_PLANES; ++plane) {
      for (int i_hit = 0; i_hit < fullhit_n[plane]; ++i_hit) {
        ...
        hist_map["h_ADC_Amp"][plane][bar]->Fill(fullhit_adc_amp_avg[plane][i_hit]);
        hist_map["h_TDC_vs_ADC_Int"][plane][bar]->Fill(fullhit_time_avg[plane][i_hit], fullhit_adc_avg[plane][i_hit]);
        hist_map["h_TDC_vs_ADC_Amp"][plane][bar]->Fill(fullhit_time_avg[plane][i_hit],
    ...
    for (int plane = 0; plane < N_PLANES; ++plane) {
      ...
      for (int i_hit = 0; i_hit < fullhit_n[plane]; ++i_hit) {
        ...
        bool is_punchthrough = (hodo_hit_punchthrough_idx[plane][i_hit] != -999);
        double diff_time;
        if (is_punchthrough) {
