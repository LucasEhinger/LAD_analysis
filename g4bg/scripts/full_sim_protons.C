// Old script: full_sim_protons.C
// New script (which produces more plots) is proton_momentum.C
...
void full_sim_protons(string inFileName, string outHistName, string treeName) {
  ...
  auto h_GEM1_p = *rdf_def.Filter("GEM1_p > 0")
                       .Histo1D({"GEM1 Momentum", "GEM1 Momentum; p [GeV/c]; Counts", n_bins, 0, 1}, "GEM1_p");
  h_GEM1_p.Write();

  auto h_GEM2_p = *rdf_def.Filter("GEM2_p > 0")
                       .Histo1D({"GEM2 Momentum", "GEM2 Momentum; p [GeV/c]; Counts", n_bins, 0, 1}, "GEM2_p");
  h_GEM2_p.Write();

  auto h_LAD_p =
      *rdf_def.Filter("LAD_p > 0").Histo1D({"LAD Momentum", "LAD Momentum; p [GeV/c]; Counts", n_bins, 0, 1}, "LAD_p");
  h_LAD_p.Write();

  auto h_GEM1_p_original =
      *rdf_def.Filter("GEM1_p > 0")
           .Histo1D({"GEM1 Original Momentum", "GEM1 Original Momentum; p [GeV/c]; Counts", n_bins, 0, 1}, "ev.p");
  h_GEM1_p_original.Write();

  auto h_GEM2_p_original =
      *rdf_def.Filter("GEM2_p > 0")
           .Histo1D({"GEM2 Original Momentum", "GEM2 Original Momentum; p [GeV/c]; Counts", n_bins, 0, 1}, "ev.p");
  h_GEM2_p_original.Write();

  auto h_LAD_p_original =
      *rdf_def.Filter("LAD_p > 0")
           .Histo1D({"LAD Original Momentum", "LAD Original Momentum; p [GeV/c]; Counts", n_bins, 0, 1}, "ev.p");
